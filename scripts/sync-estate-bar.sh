...
# manifest), then publishes only the stylesheet and font into
# site/assets/estate-bar/. The HTML fragments stay out of site/ on purpose:
# check-site.sh lints every site/**/*.html as a full page. Never hand-edit
# either copy; check-site.sh fails if they stop matching the manifest.
#
...
served="$root/site/assets/estate-bar"
...
rm -rf "$vendor" "$served"; mkdir -p "$vendor" "$served/fonts"
cp -a "$tmp/." "$vendor/"
