...
test("BLUE GOLD BLUE is an honest product page with bounded interest collection", () => {
  ...
  const faqStates = new Map([
    ...
    ["state-limit", "Known limit"],
    ["state-unknown", "Undecided"],
    ["state-future", "Future idea"],
  ]);
  for (const entry of faqDetails) {
    ...
    const state = entry.match(/class="bluegold-answer-state (state-[^"]+)">([^<]+)<\/span>/);
    assert.ok(state);
    assert.equal(state[2], faqStates.get(state[1]));
    assert.match(entry, /<div><p>[^<]/);
  }
  const faqQuestions = faqDetails.map((entry) => {
    const question = entry.match(/<summary>[\s\S]*?<span>([^<]+)<\/span><\/summary>/);
    assert.ok(question);
    return question[1];
  });
  ...
  const faqTopicTargets = [...bluegold.matchAll(/class="bluegold-faq-map"[\s\S]*?<\/nav>/g)]
    .flatMap((match) => [...match[0].matchAll(/href="#([^"]+)"/g)].map((target) => target[1]));
  assert.deepEqual(faqTopicTargets, ["faq-start", "faq-computer", "faq-data", "faq-safety", "faq-after"]);
  assert.equal(new Set(faqTopicTargets).size, faqTopicTargets.length);
  for (const target of faqTopicTargets) assert.match(bluegold, new RegExp(`id="${target}"`));
  ...
  assert.match(bluegold, /What happens to Windows when I plug in BLUE\?/);
  assert.match(bluegold, /What exactly is GOLD\?/);
  assert.match(bluegold, /What happens if I plug in GOLD before BLUE is ready\?/);
  assert.match(bluegold, /What happens when I plug BLUE in after Omarchy starts\?/);
  assert.match(bluegold, /remove BLUE, insert GOLD/);
  ...
  assert.match(bluegold, /What if the package looks opened, a device was replaced/);
  assert.match(bluegold, /Can I make my own BLUE or GOLD drive and try this now\?/);
  assert.match(bluegold, /No customer kit, installation runtime, or released safety procedure exists today/);
  assert.match(bluegold, /Can I buy BLUE GOLD BLUE or join a pilot now\?/);
  assert.match(bluegold, /has not been proven end to end on customer hardware or real customer data/);
  assert.match(bluegold, /Can I check my computer before I buy or plug anything in\?/);
  assert.match(bluegold, /Can I try Omarchy before Windows is erased\?/);
  assert.match(bluegold, /Will I have to change BIOS or UEFI settings or choose a startup drive\?/);
  assert.match(bluegold, /What happens to BitLocker recovery keys or files encrypted with Windows EFS\?/);
  assert.match(bluegold, /Exactly what will GOLD erase on the same-computer journey\?/);
  assert.match(bluegold, /Could BLUE GOLD BLUE lose my files or leave my computer unusable\?/);
  assert.match(bluegold, /can erase the Windows system disk/);
  assert.match(bluegold, /Will I need administrator access, and what about BitLocker or Secure Boot\?/);
  assert.match(bluegold, /Do I need to turn off Windows security or give anyone my administrator password\?/);
  assert.match(bluegold, /What if I have several internal drives, partitions, external drives, or network storage\?/);
  assert.match(bluegold, /Can I stop after BLUE and keep using Windows\?/);
  assert.match(bluegold, /Is BLUE a complete backup, and what will it save\?/);
  assert.match(bluegold, /Does BLUE search my whole computer, or only folders like Desktop and Documents\?/);
  assert.match(bluegold, /Can I choose what BLUE does or does not copy\?/);
  assert.match(bluegold, /Can I review my files, apps, and devices before GOLD\?/);
  ...
  assert.match(bluegold, /What about data stored inside apps, such as Outlook archives/);
  assert.match(bluegold, /Which passwords, keys, and signed-in sessions will BLUE refuse to copy\?/);
  assert.match(bluegold, /Where will my files go, and will the new computer feel familiar\?/);
  assert.match(bluegold, /Will my Wi-Fi, sound, screen, keyboard, printer, and other devices work\?/);
  ...
  assert.match(bluegold, /What if BLUE is lost, stolen, or damaged\?/);
  ...
  assert.match(bluegold, /What if Omarchy runs out of space while BLUE is putting my files back\?/);
  assert.match(bluegold, /Can I keep using my computer while BLUE saves, and when can I unplug it\?/);
  ...
  assert.match(bluegold, /Can I reuse, swap, or mix BLUE and GOLD devices between computers or migrations\?/);
  ...
  assert.match(bluegold, /Can I recover my files if the BLUE software or Intent Solutions is unavailable\?/);
  assert.match(bluegold, /Can BLUE put Windows back if I change my mind\?/);
  assert.match(bluegold, /Is BLUE GOLD BLUE an official or authorized Omarchy product\?/);
  assert.match(bluegold, /Can BLUE rescue a computer that is already failing\?/);
  assert.match(bluegold, /Can support remotely control my computer, and how would I stop access\?/);
  assert.match(bluegold, /What if Windows may be infected or ransomware is changing files/);
  ...
  assert.match(bluegold, /How will BLUE and GOLD receive updates, and how long will they be supported\?/);
  assert.match(bluegold, /Would WELCOME or its AI read my private files\?/);
  assert.match(bluegold, /Will BLUE keep working after Omarchy changes or updates\?/);
  assert.match(bluegold, /Can I complete the move with a screen reader, magnifier, voice control, or another assistive tool\?/);
  assert.match(bluegold, /Can I use this page and interest form with assistive technology\?/);
  assert.match(bluegold, /Are these real product tiers, and what will they cost\?/);
  assert.match(bluegold, /Where would it ship, and when would it arrive\?/);
  assert.match(bluegold, /What warranty, return, refund, or damaged-item replacement would apply\?/);
  assert.match(bluegold, /What am I agreeing to when I submit and confirm\?/);
  assert.match(bluegold, /What happens to my information before and after I confirm\?/);
  assert.match(bluegold, /How can I recognize the confirmation email, and what does its link do\?/);
  assert.match(bluegold, /short opaque confirmation code/);
  assert.match(bluegold, /Can I unsubscribe, withdraw my interest, or ask you to delete my information\?/);
  assert.match(bluegold, /form answers are not intentionally sent as custom analytics events/);
  assert.match(bluegold, /held server-side as an encrypted pending record/);
  ...
  assert.match(bluegold, /No OneDrive, Dropbox, or other provider-acquisition path is released/);
  assert.match(bluegold, /planned physical installation key/);
  ...
  assert.match(bluegold, /Active product build\. Not ready for customer use\./);
  assert.match(bluegold, /Engineering proof and safety validation underway/);
  ...
  assert.match(bluegold, /Higher-capacity or faster BLUE concept/);
  assert.match(bluegold, /These are not future product purchase or migration-service terms/);
